...
const fallbackRows = <T>(
  ...
  if (result.status === 'rejected') {
    console.warn(`[AUTH] Optional workflow input unavailable: ${tableName}`, result.reason);
    return [];
  }
  if (result.value.error) {
    console.warn(`[AUTH] Optional workflow input unavailable: ${tableName}`, result.value.error);
    return [];
  }
...
export const workflowIntegrityRepository = {
  async loadIntegrityInputs(firmId: string, sevenDaysAgoIso: string) {
    const [tasksRes, noticesRes, approvalsRes, payrollRes, invoicesRes, reassignRes, auditRes] = await Promise.allSettled([
    ...
    return {
      tasks: fallbackRows(tasksRes, 'tasks'),
      notices: fallbackRows(noticesRes, 'notices'),
      approvals: fallbackRows(approvalsRes, 'approval_tasks'),
      payrollRuns: fallbackRows(payrollRes, 'payroll_runs'),
      invoices: fallbackRows(invoicesRes, 'invoices'),
      reassignments: fallbackRows(reassignRes, 'task_reassignments'),
      auditLogs: fallbackRows(auditRes, 'audit_logs'),
    };
  },
};
